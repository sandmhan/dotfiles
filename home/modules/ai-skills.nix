...
let
  ...
  safeAiNameRegex = "^[A-Za-z0-9._-]+$";
  isSafeAiName = name: builtins.match safeAiNameRegex name != null && name != "." && name != "..";
  invalidAiNames = attrs: lib.filter (name: !isSafeAiName name) (lib.attrNames attrs);
  invalidSkillNames = invalidAiNames cfg.ai.skills;
  invalidRuleNames = invalidAiNames cfg.ai.rules;
  invalidNameMessage = optionName: names: ''
    ${optionName} keys must be safe basename-like path components matching
    ${safeAiNameRegex}, and must not be "." or "..". Invalid names: ${lib.concatStringsSep ", " names}
  '';
      ...
      assertions = [
        {
          ...
          message = invalidNameMessage "myHome.ai.skills" invalidSkillNames;
        }
        {
          assertion = invalidRuleNames == [ ];
          message = invalidNameMessage "myHome.ai.rules" invalidRuleNames;
        }
      ];
    }
    (lib.mkIf cfg.features.enableAISkills {
