...
Canonical deployment status, VM IDs, and IPs live in `docs/infrastructure-registry.md`.
...
DNS/internal naming remains a follow-up item; use current IPs from `docs/infrastructure-registry.md` until DNS is deployed.
...
- Update `docs/infrastructure-registry.md` with any actual deployment, IP, or VM ID change.
