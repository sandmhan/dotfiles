...
> Note: VM IDs in this roadmap are planning notes. Use `docs/architecture/infrastructure-registry.md` plus live Proxmox `qm list` output as the canonical source before creating or restoring VMs. VM ID 105 is already assigned to `agent-sandbox`; planned NVR deployment must choose a different ID.
>
