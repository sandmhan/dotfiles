...
- `home/modules/nvf/languages.nix` remains the shared/core language module. Python, web, infrastructure, and testing now use focused modules before being imported by `default.nix`; future hardening and AI work should follow that pattern.
...
| `<leader>t` | Tests/tasks via Neotest (`tn`, `tf`, `ta`, `tr`, `to`, `ts`, `tw`, `tD`) |
...
Later phases will fill in Rust, Go, Lua, SQL, workspace hardening, and AI bridge workflows. This guide does not claim those behaviors are available until their implementation tickets land.
